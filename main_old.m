function main()
    ...
    % 
    %     % if strcmp(names{y}, 'triboringC1IV') % 'perlite'
    %     % 
    ...
    if (sample == true)
            ...
            if range ~= 0
                ...
                if mean_angle_sample_clean < min_value
                    p1 = 0;  % Set probability to 0 for the min group
                    p2 = 100;  % Set probability to 100 for the max group
                elseif mean_angle_sample_clean > max_value
                    ...
                    p1 = abs(mean_angle_sample_clean - max_value)/range * 100;
                    p2 = abs(mean_angle_sample_clean - min_value)/range * 100;
                end
            else
            ...
            fprintf("Probabilité que l'échantillon appartienne au groupe %s : %.2f%%\n", names{max_index}, p1);
            fprintf("Probabilité que l'échantillon appartienne au groupe %s : %.2f%%\n", names{min_index}, p2);

            fprintf("L'échantillon appartient à %.2f%% au groupe %s et à %.2f%% au groupe %s\n", p1, names{max_index}, p2, names{min_index});
